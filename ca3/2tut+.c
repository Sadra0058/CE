...
/*free Linkedlist of each user's posts and write them to the file*/
void free_post(User *user, int *flag)
{
    if (user->head == NULL)
        return;
    /*Create a stream*/
    FILE *output;
    /*Write mode for the first time*/
    if ((int)*flag == 0)
    {
        output = fopen("posts.txt", "w");
        *flag = 1;
    }
    /*append mode for other situations*/
    else if (flag != 0)
        output = fopen("posts.txt", "a");
    Post *del;
    Post *prev = user->head;
    Post *cur = user->head->next;

    while (1) /*Write information to the file and free memmory*/
    {
        if (cur == NULL)
        {
            fprintf(output, "%s %s %i\n", prev->text, user->username, prev->likenum);
            free(prev->text);
            free(prev);
            /*close the file*/
            fclose(output);
        ...
        prev = prev->next;
        cur = cur->next;
        fprintf(output, "%s %s %i\n", del->text, user->username, del->likenum);
        free(del->text);
        free(del);
    }
}
...
void free_user(User *head)
{
    ...
    while (1)
    {
        if (cur == NULL) /*Write information to the file and free memmory*/
        {
            free_post(prev, &flag);
            free_likedpost(prev);
            fprintf(output, "%s %s %i\n", prev->username, prev->password, prev->postcount);
            free(prev->username);
            free(prev->password);
            free(prev);
            /*close the file*/
        ...
        else
        {
            ...
            free_post(del, &flag);
            free_likedpost(del);
            fprintf(output, "%s %s %i\n", del->username, del->password, del->postcount);
            free(del->username);
            free(del->password);
            free(del);
        }
...
/*It free everything and closes the program*/
void leave(User *head)
{
    free_user(head);
}
